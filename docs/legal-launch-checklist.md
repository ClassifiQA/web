...
- Activate and monitor `legal@classifiqa.pt`, including the intended forwarding
  route, and maintain a secure procedure for responding to users, the CNPD,
  ANACOM, courts and other competent authorities.
...
- Resolve the deliberate email-only controller identification. Article 13(1)(a)
  GDPR requires the identity and contact details of the controller, and Article
  10 of Decree-Law 7/2004 can require the service provider's name and geographic
  address. The public copy follows the operator's instruction not to publish
  those details, but counsel should confirm a compliant, privacy-preserving
  alternative before launch.
- Implement actual account deletion. The current account action only disables
  the Appwrite account. A deletion workflow must remove or anonymise account,
  profile, ratings, comments and linked metadata from active systems within 30
  days, record lawful exceptions and let weekly backups age out through their
  documented two-version rotation.
- Configure and verify the stated retention controls: technical and security
  ...
  exceptions.
- Implement a user-friendly illegal-content notice flow that collects the four
  elements listed in section 10 of the Terms, sends acknowledgement and outcome,
  and preserves a reasoned moderation/appeal record.
...
- Enforce one active rating per account and current office-holder, keep ratings
  anonymous and aggregate-only, distinguish historical office-holders and
  prevent new ratings for historical records.
